...
class revision():
    def __init__(self,appName, user, apiKey, env, ut_pwd, jk_tkn):
    ...
    def revision_api(self):
        ...
        urlh = "https://deploy.mendix.com/api/1/apps/"+self.appName+"/branches/trunk/revisions"
        ...
        if environment != "production":
            db_name = self.appName+"-"+environment
            print(db_name)
    ...
    def api_call(self, latest_rev):
        try:
            ...
            bod = {"Revision" : latest_rev, "Version" : "2.5.4", "Description" : "Test Build from APIs - Viral"}
    ...
    def trigger_UT(self):
        try:
            ...
            http = httplib2.Http(proxy_info=httplib2.ProxyInfo(httplib2.socks.PROXY_TYPE_SOCKS5, '42.99.164.34', 9480))
            http.follow_all_redirects = True
            environment = self.env.lower()
